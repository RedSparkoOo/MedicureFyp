...
public class DoctorNurseDashboard extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    List<String> approved_patient_img = new ArrayList<>();
    ...
    private void uploadProfileToFireBase(Uri content_uri) {
        ...
        StorageReference docFileRef = storageReference.child("Professions/" + firestoreHandler.getCurrentUser() + "/doctor_profile.jpg");
    ...
    public void upcomingAppointments() {
        ...
        FStore.collection("Approved Appointments").orderBy("ApprovedPatientName", Query.Direction.ASCENDING).addSnapshotListener(new EventListener<QuerySnapshot>() {
            ...
            public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                ...
                for (DocumentChange dc : value.getDocumentChanges()) {
                    ...
                    if (dc.equals(null))
                        ...
                        String patientId = String.valueOf(dc.getDocument().get("AppointedPatientID"));
                        ID = String.valueOf(dc.getDocument().get("AppointedDoctorId"));
                        if (firestoreHandler.getCurrentUser().equals(ID)) {
                            ...
                            if (dc != null) {
                                ...
                                if (dc.getType() == DocumentChange.Type.ADDED) {
                                    ...
                                    approved_patient_names.add(String.valueOf(dc.getDocument().get("ApprovedPatientName")));
                                    approved_patient_phone_no.add(String.valueOf(dc.getDocument().get("ApprovedPatientCell")));
                                    ...
                                    approved_patient_id.add(String.valueOf(dc.getDocument().get("AppointedPatientId")));
                                    approved_patient_img.add(String.valueOf(dc.getDocument().get("ApprovedPatientImage")));
                                    notificationsAdapter = new UpcomingNotificationsAdapter(approved_patient_names, approved_patient_phone_no, approved_appointment_date, approved_appointment_time,approved_patient_id, approved_patient_img);
